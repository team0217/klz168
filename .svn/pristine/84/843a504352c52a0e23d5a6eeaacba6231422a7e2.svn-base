 ...
 */
	operate:function(o){
		...
		url = bind.ajax_url(url);
		art.dialog.confirm('您确定购买/续费运营商？ ￥'+money, function() {
 ...
 */
	hmerchant:function(o){
		...
		var money = $(o).attr('data-money');
		url = bind.ajax_url(url);
		art.dialog.confirm('您确定购买/续费总经销商？ ￥'+money, function() {
			$.get(url, {money:money,type:type}, function(ret) {
				if(ret.status == 1){
					bind.show_message('提示信息',ret.info,'S');
					location.reload();
	...
	zmerchant:function(o){
		...
		var money = $(o).attr('data-money');
		url = bind.ajax_url(url);
		art.dialog.confirm('您确定购买/续费代理商？ ￥'+money, function() {
			$.get(url, {money:money,type:type}, function(ret) {
				if(ret.status == 1){
					bind.show_message('提示信息',ret.info,'S');
					location.reload();
    ...
    member_vip:function(o){
        ...
        art.dialog.confirm('您确定购买VIP会员？ ￥'+money, function() {
                $.get(url, {money:money,type:type}, function(ret) {
                    if(ret.status == 1){
                        bind.show_message('提示信息',ret.info,'S',{time:3, url:"/user/",refresh:true});
                    }else{
                        bind.show_message('提示信息',ret.info,'E',{time:3, url:ret.url,refresh:false});
                    }
                });
