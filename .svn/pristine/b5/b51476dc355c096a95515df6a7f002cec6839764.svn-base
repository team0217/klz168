			...
			<div class="appeal-info">
				...
				<table cellspacing="0">
					<tbody>
						...
						<tr>
							<th>活动类型:</th>
							<td><?php if($proInfo['mod']=='rebate') { ?>购物返利<?php } else { ?>免费试用<?php } ?></td>
						</tr>
						<tr>
							<th>活动时间:</th>
						...
						<tr>
							<th>已被<?php if($proInfo['mod']=='rebate') { ?>抢购<?php } else { ?>试用<?php } ?>:</th>
						...
						<tr>
							<th>折扣:</th>
							<td><?php echo $proInfo['goods_discount'];?>折</td>
						</tr>
						<?php } ?>
						<tr>
							<th>最长追加天数:</th>
							<td><?php echo $proInfo['com_day'];?>天</td>
						</tr>
						<tr>
							<th>最少追加份数:</th>
							<td><?php echo $proInfo['com_number'];?>份</td>
						</tr>
						<tr>
							<th>追加天数:</th>
...
$(function(){
    ...
    $("#com_day").formValidator({
        ...
        regexp:'intege1',
        datatype:'enum',
        onerror:'追加天数为1-<?php echo $proInfo['com_day'];?>天'
    }).functionValidator({
    	fun:function(val,elem){
    		var val = parseInt(val,10);
    		var com_day = parseInt("<?php echo $proInfo['com_day'];?>",10);
    ...
    $("#com_number").formValidator({
        ...
        regexp:'intege1',
        datatype:'enum',
        onerror:'追加份数必须大于等于<?php echo $proInfo['com_number'];?>份'
    }).functionValidator({
    	fun:function(val,elem){
    		var val = parseInt(val,10);
    		var com_number = parseInt("<?php echo $proInfo['com_number'];?>",10);
    		if (val < com_number) {
