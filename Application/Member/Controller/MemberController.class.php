...
class MemberController extends InitController {
	...
	public function detail($userid = 0){
		...
		$yeb['type'] = 'yeb';
		$yeb['num'] = array("GT",0);
		$yeb_money_total = model('member_finance_log')->where($yeb)->sum('num');
		//历史总支出
		$expend = array();
		$expend['userid'] = $userid;
		$expend['type'] = 'money';
		$expend['num'] = array("LT",0);
		$expend_money_total = model('member_finance_log')->where($expend)->sum('num')-$yeb_money_total;
		...
		//$sqlMap['type'] = 'money';
		$sqlMap['type']   = array("IN",array("money","yeb"));
		$param = I('param.');
		$search = $param['search'];
