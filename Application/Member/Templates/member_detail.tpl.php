...
<div class="explain-col">  
    ...
    <p>4.历史总支出，包括所有支出，如购买vip，缴纳保证金，转入淘金呗，转入余额宝。</p>
...
?>
    ...
    <?php }else{?>
     <tr>
		...
		<td align="left"><?php echo $v['username']?></td>
		<td align="left"><?php echo $v['cause'];?></td>
		  <?php if($v['num'] >= 0 && $v['type'] == 'money'){
          echo"<td align='left'><span class='font-fixh green'>{$v['num']}</span></td><td align='left'></td>";
        }
        else{
            $reduce = "";
            if($v['type'] == 'yeb'){$reduce = "-";}
        	echo "<td align='left'></td> 
        	      <td align='left'><span class='font-fixh'>{$reduce}{$v['num']}</span></td>";
        }
        ?>	
        <td align="left"><?php if($v['num'] >= "0" && $v['type'] == 'money'){echo "收入";}else{echo "支出";}?></td>	
