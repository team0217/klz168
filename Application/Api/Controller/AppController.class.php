...
/*获取用户绑定淘宝帐号*/
    ...
    public function getorderlists(){
        ...
        foreach ($orders as $k => $v) {
           ...
           // $r['thumb'] = $factory->product_info['thumb']; 
            ...
            //计算填写试用报告天数
            $inputtime=model('order_log')->where(array('order_id'=>$r['id'],'cause'=>array("LIKE", "填写订单号%")))->find();
            $complete=date("Y-m-d H:i:s",$inputtime['inputtime']);
            $r['trial_report_day']=strtotime($complete."+6 day");
            $r['now']=NOW_TIME;
    ...
    public function getmessage(){
        ...
        if($type == 1){//私信
            ...
            foreach ($announce_lists as $k => $v) {
                $lists2[$k]['subject'] = $v['title'];
                $lists2[$k]['content'] = $v['content'];
                $lists2[$k]['message_id'] = $v['id'];
                $lists2[$k]['inputtime'] = $v['send_time'];
                $lists2[$k]['new_type'] = 3;
            }
    ...
    public function getmessage_show(){
         ...
         if(I('param.type') ==2){
             ...
             $rs =  model('message_group')->where(array('id'=>$param['id']))->find();
             ...
             $r['subject'] =  $rs['subject'];
             $r['content'] =  $rs['content'];
             $r['inputtime'] =  $rs['inputtime'];
             $r['status'] =  $rs['status'];
      
             //是否标记已读
             $count = model('message_data')->where(array('userid' =>$param['userid'],'group_message_id' =>$rs['id']))->count();
             if(!$count){
                model('message_data')->add(array('userid' =>$param['userid'],'group_message_id' =>$rs['id'],'isdelete' =>time()));
             }
  ...
  /*联动地区*/
    ...
    public function person_cash(){
        ...
        if (IS_POST) {
            ...
            $info['ip'] = get_client_ip();
            //判断是否有申请提现
            $sign = '4-1-'.$info['userid'].'-'.$info['money'].'-'.dgmdate($info['inputtime'],'Y-m-d H:i:s');
//          $posttime = model('cash_records')->where(array('userid'=>$info['userid']))->order('cashid DESC')->find();
            $posttime = model('cash_records')->where(array('userid'=>$info['userid']))->order('inputtime desc')->limit(0,1)->select();
            if (NOW_TIME - $posttime[0]['inputtime'] <= 60) {
...
*/
    ...
    public function get_total_friend(){
        if (IS_POST) {
            ...
            $userid=$info['userid'];
            $agentCount = array();
            $info = model('member')->find($userid);
            if($info['groupid'] == 6 || $info['groupid'] == 5 || $info['groupid'] == 4 || $info['groupid'] == 2)
            {
                //一级好友
                $agentCount[1]['count'] = model('member')->where(array('agent_id' =>$userid ))->count();  
                if($info['groupid'] == 6 || $info['groupid'] == 5 || $info['groupid'] == 4)
                {
                    //二级好友
                    $sdata = model('member')->where(array('agent_id' =>$userid ))->select();
                    if(!empty($sdata))
                    {
                        $count = 0;
                        $user_ids = '';
                        foreach ($sdata as $key => $v) {
                            //二级级好友
                            $sCount= model('member')->where(array('agent_id'=>$v['userid']))->count();
                            $fdata = model('member')->where(array('agent_id'=>$v['userid']))->select();
            ...
            $this->json_function(1,'获取信息成功',$agentCount);
        }
    }
...
/*申请闪电试用*/
   ...
   public function get_webinfo(){
    ...
    public function check_money(){
        ...
        $moneys = model('member')->getFieldByUserid($this->userid,'money');
        
        $userinfo = member_info($this->userid);
    
        //判断用户是否可以再次点击
        if($userinfo['groupid'] != $type && $userinfo['groupid'] != 1){
            $groups = getcache('member_group','member');
            $name = $groups[$userinfo['groupid']]['name'];//商家级别
        ...
        if($moneys < $money){//余额不足
            ...
            //if($status == '年'){
            if($userinfo['group_endtime'] != 0){
                $group_endtime = strtotime("+20 year ",$userinfo['group_endtime']);
            }else{
            ...
            if(!$rs){
                ...
                action_finance_log($this->userid,-$money,'money',$msg,$sign,array());
                $result = model('member')->where(array('userid'=>$this->userid))->save(array('groupid'=>$type,'grouptime'=>NOW_TIME,'group_endtime'=>$group_endtime));
                //查询推荐id 是否为代理会员
                if(!empty($userinfo['agent_id']))
                {
                    //查询代理会员
                    $results = model('member')->where(array('userid'=>$userinfo['agent_id']))->find();
