...
<html>
	<head>
		<meta charset="utf-8">
		<title><?php if($type == 'all') { ?>最新消息<?php } elseif ($type == 3) { ?>官方公告<?php } elseif ($type == 2) { ?>招商公告<?php } elseif ($type == 5) { ?>VIP公告<?php } ?>-<?php echo C('WEBNAME');?></title>
			...
			<div class="wrap-and clear">
				...
				<div class="help_right fr">
					<div class="box">
						...
						<h3 class="title"><?php if($type == 5) { ?>VIP公告<?php } elseif ($type == 3) { ?>官方公告<?php } elseif ($type == 2) { ?>招商公告<?php } else { ?>最新消息<?php } ?></h3>
						
						<div class="content">
							<ul class="help_sk_jd">
								
								<?php $n=1;if(is_array($announce)) foreach($announce AS $r) { ?>
								<li class="first"><a href="<?php echo U('Announce/Index/show',array('id'=>$r[announceid],'type'=>$type));?>"><?php echo $n;?>.<?php echo $r['title'];?></a></li><br>
