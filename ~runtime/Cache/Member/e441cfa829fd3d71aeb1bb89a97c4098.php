...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<title>商家个人中心-发布任务-<?php echo C('WEBNAME');?></title>
		...
		<link rel="stylesheet" href="<?php echo THEME_STYLE_PATH;?>style/css/demo.css" /> 
	...
	<div class="i_businessman">
		...
		<div class="ibody">
			<div class="s_weiz">
				当前位置：
		<a href="<?php echo U('Member/Profile/index');?>" class="nav_active">商家中心</a>&nbsp;>编辑日赚任务
			</div>
			<!--s_weiz-->
      <?php include template('v2_merchant_left','member/common'); ?>
			<div class="s_right" >
				<div  class="title">编辑日赚任务</div>
				<div class="s_tryout5">
					<div class="part1">您编辑的活动一旦系统审批，您将不能对此试用活动进行删除，日赚任务活动平台不收取佣金,所以不退还剩余任务份数，请慎重！</div>
					<div  class="part2">
					...
					<!-- radio_set_add -->
					<form action="<?php echo U('Member/MerchantTask/task_edit');?>" name="myform" method="post" id="myform">
					<input type="hidden" value="<?php echo $id;?>" name="id"/>
						<ul class="right_release height_auto">
							<li class="shop_soure set_line_height width_100 float_left">
								<span class="font_size14 shop_value_name text_align_right">任务问题：</span>
								<input type="text" class="float_left input_width_1 height_100" name="info[title]" id="title" value="<?php echo $title;?>" placeholder="如：宝贝详情中宝贝材质中最后一句话是什么？"/>
							</li>
							<li class="shop_soure set_line_height width_100 float_left">
								<span class="font_size14 shop_value_name text_align_right">问题答案：</span>
								<input type="text" class="float_left input_width_1 height_100" name="info[answer]" id="answer" value="<?php echo $answer;?>" placeholder="答案尽可能简洁，用户回答正确后获得您设置的佣金"/>
							</li>
							...
							<li class="shop_soure set_line_height width_100 float_left">
								...
								<ul class="shop_img">
									<li class="big_small_img float_left">
										<span class="border_dddddd"><img src="<?php if(empty($thumb)) { ?><?php echo THEME_STYLE_PATH;?>style/images/signIn_14.jpg<?php } else { ?><?php echo $thumb;?> <?php } ?>" alt="" /><input type="hidden" name="info[thumb]" value="<?php echo $thumb;?>" /></span>
							...
							<li class="shop_soure set_line_height width_100 float_left">
								...
								<input type="text" class="float_left input_width_1 height_100 " name="info[keyword]" id="keyword" value="<?php echo $keyword;?>" placeholder="请输入商品关键字，多个用英文逗号（,）分割" />
							</li>
							<li class="shop_soure width_100 float_left">
								<span class="font_size14 shop_value_name text_align_right">搜索排序要求：</span>
								<label><input type="radio" value="1" name="info[sort]" <?php if($sort == 1) { ?>checked <?php } ?>/>综合</label>
						      	<label><input type="radio" value="2" name="info[sort]"  <?php if($sort == 2) { ?>checked <?php } ?>/>人气</label>
						      	<label><input type="radio" value="3" name="info[sort]"  <?php if($sort == 3) { ?>checked <?php } ?>/>销量</label>
						      	<label><input type="radio" value="4" name="info[sort]"  <?php if($sort == 4) { ?>checked <?php } ?>/>信用</label>
						      	<label><input type="radio" value="5" name="info[sort]"  <?php if($sort == 5) { ?>checked <?php } ?>/>最新</label>
						      	<label><input type="radio" value="6" name="info[sort]"  <?php if($sort == 6) { ?>checked <?php } ?>/>价格</label>
							</li>
							<li class="shop_soure set_line_height width_100 float_left">
								<span class="js_set_input font_size14 shop_value_name text_align_right">商品位置：</span>
								<input type="text" class="float_left input_width_1 height_100 " name="info[goods_address]" value="<?php echo $goods_address;?>" id="goods_address" placeholder="如第一页第二排左右" />
							</li>
							<li class="shop_soure set_line_height width_100 float_left">
								<span class="js_set_input font_size14 shop_value_name text_align_right">掌柜旺旺：</span>
								<input type="text" class="float_left input_width_1 height_100 " name="info[goods_wangwang]" id="goods_wangwang" value="<?php echo $goods_wangwang;?>" />
							</li>
							<!-- 任务份数： -->			
							<li class="shop_soure set_line_height width_100 float_left">
								<span class="font_size14 shop_value_name text_align_right">任务份数：</span>
								<input type="text" class="float_left input_width_2 height_100 " name="info[goods_number]" value="<?php echo $goods_number;?>" id="goods_number"/>份（建议按照赚佣金活动分数的1:10进行投放，保证搜索转化效果）
							</li>	
							<!-- 每份佣金 -->	
							<li class="shop_soure set_line_height width_100 float_left">
								<span class="font_size14 shop_value_name text_align_right">每份佣金：</span>
								<input type="text" name="info[goods_price]" value="<?php echo $goods_price;?>" id="goods_price" class="shop_cost1 float_left input_width_2 height_100 " />元（每份佣金最低0.1元，用户回答正确后获得此佣金）
							</li>
							<li class="shop_soure set_line_height width_100 float_left">
								...
								<ul id="goods_albums" class="shop_img float_left">
									<li class="big_small_img float_left">
										<span class="border_dddddd">
											<img src="<?php echo THEME_STYLE_PATH;?>style/images/signIn_14.jpg" width="100" height="100" />
											<input type="hidden" name="goods_albums_url[]" value="" />
										</span>
										<a href="javascript:;" name="uploadify" class="display_block float_left color_ffffff background_666666 text_align_center" id="picker">上传</a>
 									</li>
 									<?php $n=1;if(is_array($goods_albums)) foreach($goods_albums AS $g) { ?>
 									<li class="big_small_img float_left">
										<span class="border_dddddd">
											<img src="<?php echo $g['url'];?>" width="100" height="100" />
											<input type="hidden" name="goods_albums_url[]" value="<?php echo $g['url'];?>" />
										</span>
										<a href="javascript:;" onclick="task.delete_image(this);" data-id="<?php echo $id;?>" data-url="<?php echo U('Member/MerchantTask/delete_image');?>" data-name="<?php echo $g['url'];?>" class="display_block float_left color_ffffff background_666666 text_align_center">删除</a>
 									</li>
 									<?php $n++;}unset($n); ?>
								</ul>
							</li>
...
/* 发布份数 */
var task_price = <?php echo $task_price;?>//最低佣金
var task_num = <?php echo $task_num;?>//任务分数
$("#goods_number").formValidator({
	...
	fun:function(val,elem){
		if(val < Number(task_num)){
			return '任务分数需大于'+task_num;
		}else{
			return true;
		}
	}
...
$("#goods_price").formValidator({
	...
	fun:function(val,elem){
		if(Number(val) < Number(task_price)){
			return '佣金需大于'+task_price;
		}else{
			return true;
		}
	}
...
<script type="text/javascript">
	...
	$(document).ready(function() {
			...
			uploader.onUploadSuccess = function( file, response ) {
				...
				var html = '';
				if(arr[0] > 0) {
					html += '<li class="big_small_img float_left">';
					html += '<span class="border_dddddd"><img src="'+arr[1]+'" alt="" /><input type="hidden" name="goods_albums_url[]" value="'+arr[1]+'" /></span>';
					html += '<a class="display_block float_left"></a>';
					html += '</li>';
					$("#goods_albums").append(html);
				}
			}
