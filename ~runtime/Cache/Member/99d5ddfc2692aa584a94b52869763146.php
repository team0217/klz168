		...
		<div id="content">
			...
			<div class="user_index_content wrap-and clear">
				...
				<div class="fr u_index_mess user_pd_1">
					<h2 class="user_page_title"><?php echo C('REBATE_NAME');?></h2>
					<!-- 正文 -->
					<ul class="sy_list_btn clear">
						<!-- <li <?php if($state == 1) { ?>class="active"<?php } ?>><a href="<?php echo U('Member/Order/v2_manage', array('mod' => 'commission','state'=>1));?>">待审批</a></li> -->
						<li <?php if($state == 2) { ?>class="active"<?php } ?>><a href="<?php echo U('Member/Order/v2_manage', array('mod' => 'rebate','state'=>2));?>">已通过</a></li>
						<li <?php if($state == 0) { ?>class="active"<?php } ?>><a href="<?php echo U('Member/Order/v2_manage', array('mod' => 'rebate','state'=>0));?>">未通过</a></li>
					</ul>
					
					<?php if($state == 2) { ?>
						<ul class="sy_list_btn sy_list_btn_2 clear" style="height: auto; margin-top: 20px;">
							<li <?php if($state == 2 && $com_status == 2) { ?>class="active"<?php } ?>><a href="<?php echo U('Member/Order/v2_manage',array('mod'=>$mod,'state'=>'2','com_status'=>2));?>">已通过待领取<b><?php echo $no_ordersn_count;?></b></a></li>
							<li <?php if($state == 2 && $com_status == 3) { ?>class="active"<?php } ?>><a href="<?php echo U('Member/Order/v2_manage',array('mod'=>$mod,'state'=>'2','com_status'=>3));?>">订单号审核中<b><?php echo $check_report_count;?></b></a></li>
							<li <?php if($state == 2 && $com_status == 4) { ?>class="active"<?php } ?>><a href="<?php echo U('Member/Order/v2_manage',array('mod'=>$mod,'state'=>'2','com_status'=>4));?>">待修改订单号/申诉<b><?php echo $no_passreport_count;?></b></a></li>
						   
						    <li <?php if($state == 2 && $com_status == 6) { ?>class="active"<?php } ?>><a href="<?php echo U('Member/Order/v2_manage',array('mod'=>$mod,'state'=>'2','com_status'=>6));?>">申诉中<b><?php echo $appeal_count;?></b></a></li>
						     <li <?php if($state == 2 && $com_status == 5) { ?>class="active"<?php } ?>><a href="<?php echo U('Member/Order/v2_manage',array('mod'=>$mod,'state'=>'2','com_status'=>5));?>">待返款<b><?php echo $pay_count;?></b></a></li>
						    <li <?php if($state == 2 && $com_status == 7) { ?>class="active"<?php } ?>><a href="<?php echo U('Member/Order/v2_manage',array('mod'=>$mod,'state'=>'2','com_status'=>7));?>">订单完成<b><?php echo $finish_count;?></b></a></li>
									...
									<tr>
										<td class="clear">
											...
											<dl class="sy_tab_title fl">
												<dt><a href="<?php echo $r['product_info']['url'];?>"><?php echo $r['product_info']['title'];?></a></dt>
										...
										<td>
											<ul class="sy_zs_hb">
												...
												<?php if($r[product_info][bonus_price] > 0) { ?>
												<li><a href="javascript:;">闪电佣金</a></li>
												<li><a href="javascript:;">￥<?php echo $r['product_info']['bonus_price'];?></a></li>
										...
										<td>
											<ul class="sy_zs_hb">
			                                    ...
			                                    <?php } ?>
			                                     ...
			                                     <?php if($r['status']==2 && $r['act_mod'] =='rebate' && !$r['order_sn']) { ?>
				                                    <li>自动关闭倒计时</li>
				                                    <li class="c_333" id="remaining_time_<?php echo $r['id'];?>">--</li>
				                                    <script type="text/javascript">order.timer(<?php echo $r['id'];?>);</script>
				                                     <?php } elseif ($r['status'] == 2 && !$r['trial_report'] && C_READ('buyer_update_order_type','rebate')> 0 && $mod=='rebate') { ?>
				                                    <li>修改单号倒计时</li>
				                                    <li class="c_333" id="remaining_time_<?php echo $r['id'];?>">--</li>
				                                    <script type="text/javascript">order.timer(<?php echo $r['id'];?>);</script>
				                               
				                                <?php } ?>
				                                 <?php if(($r['status'] ==3 || $r['status']==5) && $r['act_mod'] =='rebate' && C_READ('seller_no_check_order','rebate') != 3) { ?>
				                                 <li >订单完成倒计时</li>
				                                   <span id="remaining_time_<?php echo $r['id'];?>">--</span>
				                                    <script type="text/javascript">order.timer(<?php echo $r['id'];?>);</script>
				                                <?php } ?>

				                                 <?php if($r['act_mod'] =='rebate' &&  $r['status'] != 7 && C_READ('seller_no_check_order','rebate') == 3 && ($r['create_time']+C_READ('seller_check_time','rebate')*86400) < NOW_TIME ) { ?>
				                                 <li >待平台审核该订单</li>
				                                <?php } ?>
											...
											<ul class="sy_zs_hb sy_zs_btn">
					                            ...
					                            <?php if($r['status'] == 2) { ?>
					                                ...
					                                <?php } else { ?>
					                                    ...
					                                    <?php if(C_READ('buyer_update_order_type','rebate')> 0 && ($mod=='rebate')) { ?>
					                                       <li><a href="javascript:;" onclick="order.fill_trade_no(<?php echo $r['id'];?><?php if($r['order_sn']) { ?>,'修改订单号'<?php } ?>);">修改订单号</a></li>
					                                    <?php } ?>
					                                <?php } ?>                               
					                            <?php } ?>

					                             <?php if($r['status'] == 3 && $mod='rebate' && C_READ('buyer_update_order_type','rebate')> 0) { ?>
					                              <li><a href="javascript:;" onclick="order.fill_trade_no(<?php echo $r['id'];?><?php if($r['order_sn']) { ?>,'修改订单号'<?php } ?>);">修改订单号</a></li>
					                             <?php } ?>
					                            ...
					                            <?php if($r['status'] == 7) { ?>
					                                <?php if($r['report_count'] < 1 && $mod == 'rebate') { ?>
												        <li><a href="javascript:order.add_show(<?php echo $r['id'];?>)">我要晒单</a></li>
					                                <?php } elseif ($mod == 'rebate') { ?>
					                                    <li><a href="<?php echo U('order/api/show_order',array('id'=>$r[id]));?>" target="_blank">查看晒单</a></li>
					                                <?php } ?>
												<?php } ?>
