...
<html>
	...
	$(document).ready(function(){
		//首页顶部移入效果
		$.ajax({
			url:"<?php echo U('Member/Attesta/v2_check_alipay');?>",
			type:'post',
			dataType:'json',
					...
					<dl class="user_pass_box_form" id="goods_albums">
						...
						<?php } ?>
							...
							<dd class="clear js_css">
								...
								<input type="text" name="name_attesta" id="name_attesta" value="<?php echo $infos['name'];?>" class="txt fl" disabled="disabled"/>
							</dd>
							<dd class="clear js_css">
							...
							<dd class="clear">
								<label for="new_pass_repeat" class="fl">支付宝二维码：</label>
...
<script type="text/javascript">
	$("#js_submit").click(function(){
		...
		$.post("<?php echo U('Member/Attesta/alipay_attesta');?>",{name_attesta:name_attesta,alipay_code:alipay_code,account:account},function(data){
						if (data.status == 1) {
							art.dialog({
							lock: true,
							fixed: true,
